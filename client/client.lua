...
function OpenClothingMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'clothing_store_menu',
        ...
        if data.current.value ~= "save" then
            ...
            saveOutfit = true
            TriggerServerEvent("qr_clothes:Save", ClothesCache, output, CurrentPrice)
            OldClothesCache = {}
...
function Change(id, category, change_type)
    if id < 1 then
        ...
        if category == "pants" or category == "boots" then
            NativeSetPedComponentEnabled(PlayerPedId(), exports['qr-appearance']:GetBodyCurrentComponentHash("BODIES_LOWER"),
                false, true, true)
        end
        if category == "shirts_full" then
            NativeSetPedComponentEnabled(PlayerPedId(), exports['qr-appearance']:GetBodyCurrentComponentHash("BODIES_UPPER"),
                false, true, true)
        end
    else
        if IsPedMale(PlayerPedId()) then
...
function OutfitsManage(outfit)
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'outfits_menu_manage',
        ...
        menu.close()
        TriggerServerEvent('qr_clothes:' .. data.current.value, outfit)
    end, function(data, menu)
        Outfits()
    end)
end
